...
def visualize_circle(img, circle, class_name, score=None, color=(255, 0, 0), thickness=2):
    r"""
    Draws a single circle on image.
    circle is expected as (cx, cy, r).
    """
    if len(circle) == 4:
        x1, y1, x2, y2 = circle
        cx = 0.5 * (x1 + x2)
        cy = 0.5 * (y1 + y2)
        radius = 0.5 * max(x2 - x1, y2 - y1)
    else:
        cx, cy, radius = circle
    cx, cy, radius = int(cx), int(cy), int(radius)
    cv2.circle(img, (cx, cy), radius, color=color, thickness=thickness)
    box_text = class_name + ' {:.2f}'.format(score) if score is not None else class_name
    ((text_width, text_height), _) = cv2.getTextSize(box_text, cv2.FONT_HERSHEY_SIMPLEX, 0.45, 1)
    text_x = max(0, cx - radius)
    text_y = max(0, cy - radius)
    cv2.rectangle(img, (text_x, text_y - int(1.3 * text_height)), (text_x + text_width, text_y), color, -1)
    cv2.putText(
        img,
        text=box_text,
        org=(text_x, max(0, text_y - int(0.3 * text_height))),
        fontFace=cv2.FONT_HERSHEY_SIMPLEX,
        fontScale=0.45,
        color=(255, 255, 255),
        lineType=cv2.LINE_AA,
