...
class YOLOV1(nn.Module):
    ...
    3. Fc layers with final layer having S*S*(4B+C) output dimensions for circles
        Final layer predicts [
            x_offset_box1,y_offset_box1,sqrt_r_box1,conf_box1, # box-1 params (4 for circles)
            ...,
            x_offset_boxB,y_offset_boxB,sqrt_r_boxB,conf_boxB, # box-B params (4 for circles)
    ...
    def __init__(self, im_size, num_classes, model_config):
        ...
        if self.use_conv:
            self.fc_yolo_layers = nn.Sequential(
                nn.Conv2d(self.yolo_conv_channels, 4 * self.B + self.C, 1),
            ) # Changed from 5 to 4 for circles
        else:
            self.fc_yolo_layers = nn.Sequential(
                nn.Flatten(),
